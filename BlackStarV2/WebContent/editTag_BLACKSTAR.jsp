...
<center>
	<input type="hidden" id="isOwner" value="${isOwner}" /> <input
		type="hidden" id="isAdmin" value="${isAdmin}" /> <input type="hidden"
		name="tagId" id="tagId" value="${tag.tagId}" />
	<table>
		<tr>
			...
			<tr>
				...
				<td><c:choose>
						<c:when test="${userType == 'admin'}">
							<input type="checkbox" name="idVerified"
								value="${tag.tagIdverified}" />
						</c:when>
						<c:otherwise>
							<input type="checkbox" name="idVerified"
								value="${tag.tagIdverified}" disabled="disabled" />
							</c:otherwise>
					</c:choose></td>
			</tr>
			<tr>
